...
def train_model(train_loader, val_loader, num_epochs=40, device='cpu', save_dir='../models'):
    ...
    print(f"Training a U-Net with a ResNet34 encoder on {device} for {num_epochs} epochs")
    ...
    for epoch in range(num_epochs):
        ...
        if val_iou > best_iou:
            ...
            print(f"Best result so far — saving it (IoU {best_iou:.4f}).")

    torch.save(model.state_dict(), f"{save_dir}/final_model.pth")
    plot_training_history(history, f"{save_dir}/training_history.png")

    print(f"Training done. Best validation IoU I hit was {best_iou:.4f}, saved in {save_dir}/.")
